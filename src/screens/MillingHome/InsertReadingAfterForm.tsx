...
export function InsertReadingAfterForm({
  afterReadingId,
  afterStation,
  onClose,
  onSaved,
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSubmitting(true)
    try {
      await insertWidthReadingBetween(afterReadingId, stationValue, widthValue)
      onSaved?.()
      onClose()
    } catch (err) {
      // insert_width_reading_between raises a specific "No room left..."
      // message when the two neighboring sequence values are already as
      // close as the column's numeric(10,3) precision allows — surfaces
      // here as-is via extractErrorMessage rather than a generic failure.
